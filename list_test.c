...
char *test_init_works(void)
{
  ...
  list_init(list, NULL);
  mu_test("List should have size 0", list_size(list) == 0, cleanup);
  mu_test("List's head should be NULL", list_head(list) == NULL, cleanup);
  mu_test("List's tail should be NULL", list_tail(list) == NULL, cleanup);
...
char *test_remove_works(void)
{
  ...
  mu_test("New Tail is 3", 
          *(int *)list_data(list_tail(list)) == 3, 
          cleanup);
